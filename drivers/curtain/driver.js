...
const deviceTypes = ["22000000"];

class curtainDriver extends Driver {
  ...
  async onPairListDevices() {
    ...
    if (ip && key) {
      ...
      JSON.parse(result)['data'].forEach(function(item) {
        ...
        devices.push({ name: 'curtain-'+item['mac'].substr(item['mac'].length-8,8), data: { id: item['mac'], deviceType: item["deviceType"] }})
        }
      })
      return devices;
    }
